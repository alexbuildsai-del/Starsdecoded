...
import {
  QUADRANT_NAMES, arcLabelPath, aspectStrength, assignLanes, houseSign, opposite,
...
export function NatalWheel({
  ...
  const house = selectedHouse ?? internalHouse;
  ...
  return (
    ...
    <div className={`grid gap-4 items-start${renderHouse ? " lg:grid-cols-[minmax(0,1fr)_minmax(0,20rem)]" : ""}`}>
      ...
      >
        ...
        {/* Sign band, house-number ring, and the house hit area between them. */}
        {Array.from({ length: 12 }, (_, i) => i + 1).map((h) => {
          ...
          return (
            ...
            >
              <path
                d={wedgePath(c, c, r.tick, r.houseOuter, b0, b1)}
              ...
              <path
                d={wedgePath(c, c, r.houseOuter, r.houseInner, b0, b1)}
                fill="none"
                stroke="hsl(var(--brass) / 0.16)"
                strokeWidth={1}
              />
              <text
                x={numberAt.x}
                y={numberAt.y + PLATE * 0.011}
                textAnchor="middle"
                fontFamily="IBM Plex Mono, monospace"
                fontSize={PLATE * 0.026}
                fill={selected ? BRASS : "hsl(var(--foreground) / 0.4)"}
              >
                {h}
              </text>
            </g>
      ...
      {renderHouse && <div className="min-w-0">{renderHouse(house)}</div>}
    </div>
  );
}
