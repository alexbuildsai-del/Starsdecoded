 ...
 * asking who the parent is; one call to action. No birth form, ever.
 */
...
import { LENSES, PARENT_QUESTION, lensInfo } from "@/lib/lenses";
...
interface Selection { a: string; b: string; lens: Lens; parent: "A" | "B" }
...
export function CompatibilityPicker({ reports: given }: { reports?: ReportSummary[] }) {
  ...
  useEffect(() => { rememberSelection({ a, b, lens: lens || undefined, parent }); }, [a, b, lens, parent]);

  const info = lens ? lensInfo(lens) : null;
  const nameOf = (id: string) => reports.find((r) => r.id === id)?.name ?? "";
  const ready = a && b && a !== b && lens && !unpickable(reports.find((r) => r.id === a)!) && !unpickable(reports.find((r) => r.id === b)!);
  const noCredit = credits.data ? credits.data.available === 0 : false;

  function submit() {
    if (!ready || !lens) return;
    create.mutate({ data: { reportAId: a, reportBId: b, lens, ...(info?.asksParent ? { parent } : {}) } }, {
      onSuccess: (res) => {
        ...
        <div className="mt-1 grid gap-2 sm:grid-cols-3">
          {LENSES.map((l) => (
            <label key={l.lens} className={`cursor-pointer rounded-xl border px-3 py-2 ${lens === l.lens ? "border-primary/60 bg-primary/10" : "border-border/60 hover:border-border"}`}>
              ...
              <span className="mt-0.5 block text-xs leading-snug text-muted-foreground">{l.chapterSeven} · {l.chapterEight}</span>
            </label>
          ))}
        </div>
      </div>
