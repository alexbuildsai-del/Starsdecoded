...
chapter and the landing: https://claude.ai/artifact/A7WmYgjNgncD8ij9ppU6C4. Status: **locked 2026-09-25**,
ADR-78 to 80.
...
one small motion to its own stars. The round reverts the trial; none of it ships.
...
- **One micro motion on existing stars**, in `ReportSky` (both variants): a *slow pulse*. The ten stars with the largest radius in each sky breathe,
    opacity .55 to 1, radius ×1 to ×1.3 with a soft glow, one sine cycle of 6 to 10 s each
    with a seeded phase, so they never pulse together. Every other star stays still. A star that has landed on the ring never moves.
...
- **The app screens**: today's `.bg-stars` pattern stays, with a small
...
- Trial clean-up: the App-level `NightSky` mount, the page-root background removals and the
  `.rp-*` CSS edits from 50b48c2 are reverted and `night-sky.tsx` is deleted.
...
5. App screens with `.bg-stars` show the same pulse; none loses its current look.
...
## Resolved
1. Twinkle or slow pulse: **slow pulse** (the Owner, 2026-09-25).
2. App screens: **today's look with the same pulse** (taken at its default).

## Decisions recorded
- ADR-78: the report's skies stay as ADR-59 has them; the Milky Way band is not used.
- ADR-79: one ambient slow pulse on each sky's own stars (the ten largest, 6 to 10 s); none under
  reduced motion; ambient motion is not a chapter move. Amends §9's motion line.
- ADR-80: the app screens keep `.bg-stars` with the same pulse; no pointer drift.
