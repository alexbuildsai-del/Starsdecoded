# R02 report — the wheel becomes a chart, claims cite in place, Observatory lands

Ran 2026-09-17 on `claude/great-rubin-2ernfv`; the session was pinned there, so
no `round/R02` branch was cut. Plan: `docs/rounds/R02-plan.md`. Spec:
...
- **R02-07 Content removed** — USER-FACING. Twelve hand-written house
  descriptions and every invented angle fallback are gone (ADR-18), as the Owner
  was told.
- **R02-08 Geometry asserted** — INTERNAL. `web/` gets `vitest` over the pure
  geometry module only, twelve assertions (MB-41 provisional).
