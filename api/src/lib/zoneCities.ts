/**
 * Every IANA time zone's principal city and where it is, so the waitlist page
 * can show the sky over the visitor's city without asking where they are.
 * Taken from tzdb 2025b: zone.tab and zone1970.tab for the cities, the links in
 * tzdata.zi for the older names browsers still report ("Asia/Calcutta"). The
 * tz database is in the public domain.
 */
export const ZONE_CITIES: Readonly<Record<string, readonly [lat: number, lon: number]>> = {
...
export const ZONE_LINKS: Readonly<Record<string, string>> = {
